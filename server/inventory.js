...
module.exports = function createInventoryRouter({ oauthClient, hasGoogleConfig }) {
  ...
  router.put('/comments/settings', (req, res, next) => { try { const current = readSettings(); const nextValue = { ...current, enabled: Boolean(req.body.enabled), text: String(req.body.text || '').slice(0, 10000), categories: req.body.categories || current.categories || {}, channels: req.body.channels || current.channels || {} }; saveSettings(nextValue); res.json(nextValue); } catch (error) { next(error); } });
